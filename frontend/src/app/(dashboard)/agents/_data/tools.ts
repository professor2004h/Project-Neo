export const DEFAULT_AGENTPRESS_TOOLS: Record<string, { enabled: boolean; description: string; icon: string; color: string }> = {
    ...
    'sb_pandoc_tool': { enabled: false, description: 'Document format conversion between Markdown, HTML, PDF, DOCX, LaTeX, EPUB and more using Pandoc', icon: '📄', color: 'bg-purple-100 dark:bg-purple-800/50' },
...
export const getToolDisplayName = (toolName: string): string => {
    const displayNames: Record<string, string> = {
      ...
      'sb_pandoc_tool': 'Document Converter',
