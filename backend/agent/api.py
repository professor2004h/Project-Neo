...
class AgentStartRequest(BaseModel):
    # Will be set to default model in the endpoint
...
async def stop_agent_run(agent_run_id: str, error_message: Optional[str] = None):
    ...
    logger.debug(
        f"Successfully initiated stop process for agent run: {agent_run_id}")
...
async def start_agent(
    ...
    if effective_agent_id:
        ...
        if not agent_result.data:
            ...
            if agent_data.get('current_version_id'):
                try:
                    ...
                    logger.warning(
                        f"[AGENT LOAD] Failed to get version data: {e}")
        ...
        logger.debug(
            f"[AGENT LOAD] No effective_agent_id, will try default agent")

    if not agent_config:
        logger.debug(
            f"[AGENT LOAD] No agent config yet, querying for default agent")
    ...
    logger.debug(
        f"[AGENT LOAD] Final agent_config: {agent_config is not None}")
    if agent_config:
        logger.debug(
            f"[AGENT LOAD] Agent config keys: {list(agent_config.keys())}")
        logger.debug(
    ...
    model_check_task = asyncio.create_task(
        can_use_model(client, account_id, model_name))
    billing_check_task = asyncio.create_task(
        check_billing_status(client, account_id))
    limit_check_task = asyncio.create_task(
    ...
    if not can_use:
        raise HTTPException(status_code=403, detail={
                            "message": model_message, "allowed_models": allowed_models})
...
async def get_agent_runs(thread_id: str, user_id: str = Depends(get_current_user_id_from_jwt)):
    ...
    logger.debug(
        f"Found {len(agent_runs.data)} agent runs for thread: {thread_id}")
...
async def get_thread_agent(thread_id: str, user_id: str = Depends(get_current_user_id_from_jwt)):
    ...
    try:
        ...
        if recent_agent_result.data:
            ...
            recent_version_id = recent_agent_result.data[0].get(
                'agent_version_id')
            agent_source = "recent"
            logger.debug(
        ...
        if agent_data.get('current_version_id'):
            try:
                ...
                current_version = AgentVersionResponse(
                    ...
                    configured_mcps=current_version_data.get(
                        'configured_mcps', []),
                    custom_mcps=current_version_data.get('custom_mcps', []),
                    agentpress_tools=current_version_data.get(
                        'agentpress_tools', {}),
        ...
        return {
            "agent": AgentResponse(
                ...
                updated_at=agent_data.get(
                    'updated_at', agent_data['created_at']),
    ...
    async def stream_generator(agent_run_data):
        ...
        try:
            ...
            if initial_responses_json:
                initial_responses = [json.loads(r)
            ...
            # 2. Check run status
            current_status = agent_run_data.get(
                'status') if agent_run_data else None
            ...
            async def listen_messages():
                ...
                tasks = [asyncio.create_task(response_reader.__anext__(
                ...
                while not terminate_stream:
                    ...
                    for task in done:
                        try:
                            ...
                            logger.error(
                                f"Error in listener for {agent_run_id}: {e}")
            ...
            while not terminate_stream:
                try:
                    ...
                    logger.error(
                        f"Error in stream generator main loop for {agent_run_id}: {loop_err}", exc_info=True)
                    terminate_stream = True
            ...
            logger.error(
                f"Error setting up stream for agent run {agent_run_id}: {e}", exc_info=True)
            ...
            logger.debug(
                f"Streaming cleanup complete for agent run: {agent_run_id}")
...
async def initiate_agent_with_files(
    prompt: str = Form(...),
    # Default to None to use default model
    ...
    if agent_id:
        logger.debug(
            f"[AGENT INITIATE] Querying for specific agent: {agent_id}")
        # Get agent
        ...
        if agent_data.get('current_version_id'):
            try:
                ...
                logger.warning(
                    f"[AGENT INITIATE] Failed to get version data: {e}")
    ...
    logger.debug(
        f"[AGENT INITIATE] Final agent_config: {agent_config is not None}")
    if agent_config:
        logger.debug(
            f"[AGENT INITIATE] Agent config keys: {list(agent_config.keys())}")
...
async def get_agents(
    ...
    sort_by: Optional[str] = Query(
        "created_at", description="Sort field: name, created_at, updated_at, tools_count"),
    sort_order: Optional[str] = Query(
        "desc", description="Sort order: asc, desc"),
    has_default: Optional[bool] = Query(
        None, description="Filter by default agents"),
    ...
    try:
        ...
        query = client.table('agents').select(
            '*', count='exact').eq("account_id", user_id)
        ...
        agent_version_map = {}
        version_ids = list({agent['current_version_id']
                           for agent in agents_data if agent.get('current_version_id')})
        if version_ids:
            try:
                ...
                logger.warning(
                    f"Failed to batch load versions for agents: {e}")
        ...
        if has_mcp_tools is not None or has_agentpress_tools is not None or tools:
            ...
            if tools:
                ...
                if isinstance(tools, str):
                    ...
                    tools_filter = [str(tool).strip()
                                    for tool in tools if str(tool).strip()]
                else:
                    logger.warning(
            ...
            for agent in agents_data:
                ...
                if has_mcp_tools is not None:
                    has_mcp = bool(
                        configured_mcps and len(configured_mcps) > 0)
                ...
                if has_agentpress_tools is not None:
                    has_enabled_tools = any(
                        tool_data and isinstance(
                            tool_data, dict) and tool_data.get('enabled', False)
        ...
        if sort_by == "tools_count":
            ...
            agents_data.sort(key=get_tools_count,
            ...
            if version_dict:
                try:
                    current_version = AgentVersionResponse(
                        ...
                        configured_mcps=version_dict.get(
                            'configured_mcps', []),
                        custom_mcps=version_dict.get('custom_mcps', []),
                        agentpress_tools=version_dict.get(
                            'agentpress_tools', {}),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch agents: {str(e)}")
    ...
    try:
        ...
        if agent_data.get('current_version_id'):
            try:
                ...
                logger.warning(
                    f"Failed to get version data for agent {agent_id}: {e}")
        ...
        logger.error(
            f"Error fetching agent {agent_id} for user {user_id}: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch agent: {str(e)}")
...
async def export_agent(agent_id: str, user_id: str = Depends(get_current_user_id_from_jwt)):
    ...
    try:
        ...
        sanitized_config = template_service._fallback_sanitize_config(
            full_config)
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to export agent: {str(e)}")
...
async def analyze_json_for_import(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Failed to analyze JSON: {str(e)}")
...
async def import_agent_from_json(
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Failed to import agent: {str(e)}")
...
async def create_agent(
    ...
    try:
        ...
        logger.debug(
            f"Created agent {agent['agent_id']} with v1 for user: {user_id}")
        return AgentResponse(
            agent_id=agent['agent_id'],
            name=agent['name'],
            description=agent.get('description'),
...
async def update_agent(
    ...
    try:
        ...
        if current_version_data is None:
            ...
            try:
                ...
                if triggers_result.data:
                    ...
                    for trigger in triggers_result.data:
                        ...
                        if 'config' in trigger_copy and isinstance(trigger_copy['config'], str):
                            try:
                                trigger_copy['config'] = json.loads(
                                    trigger_copy['config'])
                ...
                if version_result.data:
                    ...
                    logger.debug(
                        f"Created initial version for agent {agent_id}")
                else:
        ...
        def values_different(new_val, old_val):
            ...
            try:
                new_json = json.dumps(
                    new_val, sort_keys=True) if new_val is not None else None
                old_json = json.dumps(
                    old_val, sort_keys=True) if old_val is not None else None
        ...
        current_system_prompt = agent_data.system_prompt if agent_data.system_prompt is not None else current_version_data.get(
            'system_prompt', '')
        current_configured_mcps = agent_data.configured_mcps if agent_data.configured_mcps is not None else current_version_data.get(
            'configured_mcps', [])
        ...
        current_agentpress_tools = agent_data.agentpress_tools if agent_data.agentpress_tools is not None else current_version_data.get(
            'agentpress_tools', {})
        current_avatar = agent_data.avatar if agent_data.avatar is not None else existing_data.get(
            'avatar')
        current_avatar_color = agent_data.avatar_color if agent_data.avatar_color is not None else existing_data.get(
            'avatar_color')
        new_version_id = None
        if needs_new_version:
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Failed to create new agent version: {str(e)}")
        ...
        if update_data:
            try:
                ...
                if not update_result.data:
                    raise HTTPException(
                        status_code=500, detail="Failed to update agent - no rows affected")
            except Exception as e:
        ...
        if not updated_agent.data:
            raise HTTPException(
                status_code=500, detail="Failed to fetch updated agent")
        ...
        logger.error(
            f"Error updating agent {agent_id} for user {user_id}: {str(e)}")
        raise HTTPException(
            status_code=500, detail=f"Failed to update agent: {str(e)}")
...
async def delete_agent(agent_id: str, user_id: str = Depends(get_current_user_id_from_jwt)):
    ...
    try:
        ...
        if agent['is_default']:
            raise HTTPException(
                status_code=400, detail="Cannot delete default agent")

        if agent.get('metadata', {}).get('is_suna_default', False):
            raise HTTPException(
                status_code=400, detail="Cannot delete Suna default agent")
        ...
        try:
            ...
            if triggers_result.data:
                logger.debug(
                    f"Cleaning up {len(triggers_result.data)} triggers for agent {agent_id}")
                ...
                for trigger_record in triggers_result.data:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Successfully cleaned up trigger {trigger_id}")
                    except Exception as e:
                        logger.warning(
                            f"Failed to clean up trigger {trigger_id}: {str(e)}")
                        # Continue with other triggers even if one fails
        except Exception as e:
            logger.warning(
                f"Failed to clean up triggers for agent {agent_id}: {str(e)}")
        ...
        if not delete_result.data:
            ...
            raise HTTPException(
                status_code=403, detail="Unable to delete agent - permission denied or agent not found")
    ...
    try:
        ...
        if not profile:
            ...
            try:
                ...
                logger.warning(
                    f"Could not check user's profiles: {str(debug_e)}")

            raise HTTPException(
                status_code=404, detail=f"Profile {profile_id} not found or access denied")

        if not profile.is_connected:
            raise HTTPException(
                status_code=400, detail="Profile is not connected")
        ...
        try:
            ...
            for tool in available_tools:
                is_enabled = any(tools_match(tool.name, stored_tool)
                                 for stored_tool in enabled_tools)
        ...
        logger.error(
            f"Error getting Pipedream tools for agent {agent_id}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def update_pipedream_tools_for_agent(
    ...
    try:
        ...
        if updated:
            ...
            logger.debug(
                f"Successfully updated agent configuration for {agent_id}")
        ...
        logger.error(
            f"Error updating Pipedream tools for agent {agent_id}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_custom_mcp_tools_for_agent(
    ...
    logger.debug(
        f"Getting custom MCP tools for agent {agent_id}, user {user_id}")
    try:
        client = await db.client
        ...
        if not mcp_url:
            raise HTTPException(
                status_code=400, detail="X-MCP-URL header is required")
        ...
        if 'X-MCP-Headers' in request.headers:
            import json
            try:
                mcp_config['headers'] = json.loads(
                    request.headers['X-MCP-Headers'])
            except json.JSONDecodeError:
        ...
        tools = []
        enabled_tools = existing_mcp.get(
            'enabledTools', []) if existing_mcp else []
        ...
        logger.error(
            f"Error getting custom MCP tools for agent {agent_id}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def update_custom_mcp_tools_for_agent(
    ...
    logger.debug(
        f"Updating custom MCP tools for agent {agent_id}, user {user_id}")
    ...
    try:
        ...
        if not updated:
            if mcp_type == 'composio':
                try:
                    ...
                    raise HTTPException(
                        status_code=400, detail=f"Failed to get Composio profile: {str(e)}")
            else:
                new_mcp_config = {
        ...
        try:
            ...
            new_version = await version_service.create_version(
                ...
                custom_mcps=custom_mcps,
                agentpress_tools=agent_config.get(
            ...
            logger.error(
                f"Failed to create version for custom MCP tools update: {e}")
            raise HTTPException(
        ...
        logger.error(
            f"Error updating custom MCP tools for agent {agent_id}: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_agent_tools(
    ...
    if not await is_enabled("custom_agents"):
        raise HTTPException(
            status_code=403, detail="Custom agents currently disabled")

    logger.debug(
        f"Fetching enabled tools for agent: {agent_id} by user: {user_id}")
    client = await db.client
    ...
    if agent.get('current_version_id'):
        try:
            ...
            logger.warning(
                f"Failed to fetch version data for tools endpoint: {e}")
    ...
    for name, enabled in agentpress_tools_config.items():
        is_enabled_tool = bool(enabled.get('enabled', False)) if isinstance(
            enabled, dict) else bool(enabled)
        agentpress_tools.append({"name": name, "enabled": is_enabled_tool})

    mcp_tools = []
    for mcp in configured_mcps + custom_mcps:
        server = mcp.get('name')
        enabled_tools = mcp.get('enabledTools') or mcp.get(
            'enabled_tools') or []
        for tool_name in enabled_tools:
            mcp_tools.append(
                {"name": tool_name, "server": server, "enabled": True})
    ...
    try:
        ...
        if thread.get('project_id'):
            ...
            if project_result.data:
                ...
                logger.debug(
                    f"[API] Raw project from DB for thread {thread_id}")
                project_data = {
                    "project_id": project['project_id'],
                    "name": project.get('name', ''),
                    "description": project.get('description', ''),
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch thread: {str(e)}")
...
async def get_thread_messages(
    ...
    user_id: str = Depends(get_current_user_id_from_jwt),
    order: str = Query(
        "desc", description="Order by created_at: 'asc' or 'desc'")
):
    """Get all messages for a thread, fetching in batches of 1000 from the DB to avoid large queries."""
    logger.debug(
        f"Fetching all messages for thread: {thread_id}, order={order}")
    client = await db.client
    await verify_thread_access(client, thread_id, user_id)
    try:
        batch_size = 1000
        ...
        while True:
            ...
            logger.debug(
                f"Fetched batch of {len(batch)} messages (offset {offset})")
...
async def add_message_to_thread(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to add message: {str(e)}")
...
async def create_message(
    ...
    try:
        ...
        logger.debug(
            f"Created message: {message_result.data[0]['message_id']}")
        return message_result.data[0]
...
async def update_agent_custom_mcps(
    ...
    try:
        ...
        if not new_custom_mcps:
            raise HTTPException(
                status_code=400, detail="custom_mcps array is required")
...
async def export_presentation(
    ...
    try:
        ...
        if not presentation_name:
            raise HTTPException(
                status_code=400, detail="presentation_name is required")

        if not project_id:
            raise HTTPException(
                status_code=400, detail="project_id is required")
...
async def upload_agent_profile_image(
    ...
    try:
        ...
        logger.error(
            f"Failed to upload agent profile image for user {user_id}: {e}")
        raise HTTPException(
