...
from agent.tools.sb_pandoc_tool import SandboxPandocTool
...
async def run_agent(
    ...
    if enabled_tools is None:
        ...
        thread_manager.add_tool(SandboxPandocTool, project_id=project_id, thread_manager=thread_manager)
        ...
        if enabled_tools.get('sb_vision_tool', {}).get('enabled', False):
            thread_manager.add_tool(SandboxVisionTool, project_id=project_id, thread_id=thread_id, thread_manager=thread_manager)
        if enabled_tools.get('sb_pandoc_tool', {}).get('enabled', False):
            thread_manager.add_tool(SandboxPandocTool, project_id=project_id, thread_manager=thread_manager)
